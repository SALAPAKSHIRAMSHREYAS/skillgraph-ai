...
import { CheckCircle2, CircleDashed, GitFork, TriangleAlert } from "lucide-react"
    ...
    <div className="overflow-hidden rounded-2xl border border-border/60 bg-card backdrop-blur-xl">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border/50 px-6 py-5">
        ...
        <span className="font-mono text-[11px] tracking-tight text-muted-foreground">
          {repositories.length} of {totalRepos} shown · sorted by weight
        </span>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[720px] border-collapse text-left">
          <caption className="sr-only">
            Repositories scanned for {profile?.handle ?? "user"} with language, originality and verification status
          </caption>
          <thead>
            <tr className="border-b border-border/50">
              {["Repository", "Language", "Originality", "Commits", "Last push", "Status"].map((h) => (
                <th
                  key={h}
                  scope="col"
          ...
          <tbody>
            {repositories.length === 0 ? (
              ...
              repositories.map((repo: any, i: number) => {
                ...
                // Parse originality to guarantee a clean numeric percentage
                ...
                return (
                  <motion.tr
                    ...
                    className="border-b border-border/40 transition-colors duration-300 last:border-0 hover:bg-secondary/25"
                  >
